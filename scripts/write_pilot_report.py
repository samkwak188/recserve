"""Fail closed on missing/failed receipts, then consolidate local pilot evidence."""
import datetime
import hashlib
import json
...
def main():
    ...
    names = ['pilot-windows-python', 'pilot-python', 'pilot-ctest', 'pilot-demo', 'pilot-movielens',
             'ownership-test-build', 'ownership-test-build-gpu', 'ownership-test-build-check-address',
             'ownership-test-build-check-undefined', 'ownership-test-build-check-thread',
             'ownership-container-build', 'ownership-container-runtime', 'ownership-gpu-investigation']
    receipts = {}
    for name in names:
        receipt = json.loads((root/'.cache/logs'/f'{name}.json').read_text())
        if receipt.get('status') != 'completed' or receipt.get('exit_code') != 0:
            raise RuntimeError(f'unpassed required gate: {name}')
        receipts[name] = receipt
    source_files = [*root.glob('recserve_pilot/*.py'), root/'apps/server.cpp',
                    root/'include/recserve/gpu_batcher.hpp', root/'include/recserve/histogram.hpp']
    result = dict(utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                  ...
                  receipts=receipts, all_listed_local_commands_succeeded=True,
                  provenance_note='Source hashes are report-time snapshots; receipts identify their original commands/times. GPU study records its own measurement-time hashes.')
    (root/'results/pilot-validation.json').write_text(json.dumps(result, indent=2)+'\n')
    def count(name):
        log = (root/'.cache/logs'/f'{name}.log').read_text(errors='replace')
        return int(re.search(r'Ran (\d+) tests?', log)[1])
    lines = ['# Pilot execution report', '',
             ...
             '## Executed checks', '',
             f'- {count("pilot-python")} Python discovery tests passed in WSL.',
             f'- {count("pilot-windows-python")} pilot tests passed on native Windows Python 3.13.',
             ...
             '## Reproduce', '',
             '```powershell', '.\\scripts\\Check-Pilot.ps1', '```', '',
             '```bash', 'bash scripts/check_ownership_runtime.sh tests-only',
    ...
    (root/'results/PILOT_EXECUTION_REPORT.md').write_text('\n'.join(lines), encoding='utf-8')
    print(json.dumps(dict(passed=True, receipts=len(receipts), python_tests=count('pilot-python'),
                         windows_pilot_tests=count('pilot-windows-python'))))
